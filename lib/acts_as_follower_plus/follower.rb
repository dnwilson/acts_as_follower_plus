module ActsAsFollowerPlus #:nodoc:
  module Follower
    ...
    module InstanceMethods
      ...
      def pending?(followable)
        0 < Follow.unblocked.pending.for_follower(self).for_followable(followable).count
      end
      ...
      def follow(followable)
        if self != followable
          self.follows.find_or_create_by(followable_id: followable.id, followable_type: parent_class_name(followable), pending: !!followable.private?)
        end
      end
      ...
      def follows_by_type(followable_type, options={})
        self.follows.unblocked.approved.includes(:followable).for_followable_type(followable_type).apply_finder_options(options, true)
      end
      ...
      def follows_pending_by_type(followable_type, options={})
        self.follows.unblocked.pending.includes(:followable).for_followable_type(followable_type).apply_finder_options(options, true)
      end
